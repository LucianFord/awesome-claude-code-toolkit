...
| [agent-pack](https://github.com/LucianFord/agent-pack) | new | 12 production-tested subagent configs + 3 multi-agent orchestration workflows for Claude Code and Kimi Code. Each agent ships dispatch conditions, known failure modes, and acceptance criteria. Found every documented vulnerability class on the OWASP NodeGoat benchmark, zero misses (single-run methodology in BENCHMARK.md). 3 agents free, MIT |
